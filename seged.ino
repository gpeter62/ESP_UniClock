...
void inline DWSDA(byte val) {
  if (val) WRITE_PERI_REG( PIN_OUT_SET, SDA_BIT );
  else   WRITE_PERI_REG( PIN_OUT_CLEAR, SDA_BIT );
}

void inline DWSCL(byte val) {
  if (val) WRITE_PERI_REG( PIN_OUT_SET, SCL_BIT );
  else   WRITE_PERI_REG( PIN_OUT_CLEAR, SCL_BIT );
}
